...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        single_dis_return = sum([r * self.gamma ** t for t, r in enumerate(rewards)])
        dis_return = [single_dis_return for _ in rewards]
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        n = len(rewards)
        dis_return = [0] * n
        # calculate in the reverse order
        for t in reversed(range(n)):
            dis_return[t] = rewards[t] + (self.gamma * dis_return[t + 1] if t < n - 1 else 0)
