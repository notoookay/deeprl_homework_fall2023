...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # Average the two critics' predictions
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action_distribution.sample() # (batch_size, action_dim)
            ...
            # Unsqueeze done to match the shape of next_qs
            done = done[None].expand((self.num_critic_networks, batch_size)).contiguous()
            next_qs[done] = 0
        ...
        return {
            ...
            "critic_entrpy": next_action_entropy.mean().item() if self.use_entropy_bonus and self.backup_entropy else 0.0
        }
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        actions = action_distribution.rsample() # (batch_size, action_dim)
        entropy = -action_distribution.log_prob(actions).mean(dim=-1) # (batch_size,)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = action_distribution.sample((self.num_actor_samples,)) # (num_actor_samples, batch_size, action_dim)
            assert action.shape == (
            ...
            obs = obs[None].expand((self.num_actor_samples, batch_size, -1)).contiguous() # (num_actor_samples, batch_size, obs_dim)
            q_values = self.critic(obs, action)
            ...
            q_values = torch.mean(q_values, axis=0) # (num_actor_samples, batch_size)
            advantage = q_values

        # Do REINFORCE: calculate log-probs and use the Q-values
        # TODO(student)
        log_probs = action_distribution.log_prob(action) # (num_actor_samples, batch_size)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # Note: Think about whether to use .rsample() or .sample() here...
        action = action_distribution.rsample((self.num_actor_samples,)) # (num_actor_samples, batch_size, action_dim)

        # TODO(student): Compute Q-values for the sampled state-action pair
        obs = obs[None].expand((self.num_actor_samples, batch_size, -1)).contiguous() # (num_actor_samples, batch_size, obs_dim)
        q_values = self.critic(obs, action) # (num_critic_networks, num_actor_samples, batch_size)
        q_values = torch.mean(q_values, axis=0) # (num_actor_samples, batch_size)
